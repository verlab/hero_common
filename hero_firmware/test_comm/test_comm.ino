...
void setup() {
  ...
  /* ROS LOG */
  sprintf(buf,"\33[96m [TestComm] Founded %s! \33[0m", hero_name.c_str());
  nh.loginfo(buf);
}
...
void loop(){
      ...
      if ((millis() - tunning_steady_state) > 10000){ /* 10 seconds to steady state */
        ...
        if (l_done && r_done){
          ...
          if (lkp > 100 && lkp < 2500){ // * Tuning is successed
            ...
            nh.loginfo("\33[91m [TestComm] Left Motor Tuning Failure! Using default values.\33[0m");
          }
          sprintf(buf,"\33[96m [TestComm] Left Motor Best PID params (%f, %f, %f) \33[0m", lkp, lki, lkd);
          nh.loginfo(buf);
          ...
          if (rkp > 100 && rkp < 2500){ // * Tuning is successed
            ...
            nh.loginfo("\33[91m [TestComm] Right Motor Tuning Failure! Using default values. \33[0m");
          }
          sprintf(buf,"\33[96m [TestComm] Right Motor Best PID params: (%f, %f, %f) \33[0m", rkp, rki, rkd);
            nh.loginfo(buf);
        }
      }
    ...
    motorPositionRight = random(100);
    motorPositionLeft = random(100);
  
    /* ROS INFOS */
    if ((millis() - log_timer) > 3000){
      log_timer = millis();
      if (tuning)
        sprintf(buf,"\33[96m[TestComm][%s] Tuning PID! Please wait (200 seconds)... %d ms\33[0m", hero_name.c_str(), (millis()-tunning_steady_state)/1000);
      else
        sprintf(buf,"\33[96m[TestComm][%s] Conected at time %d\33[0m", hero_name.c_str(), millis());
      nh.loginfo(buf);
    }
...
void readIMU(){
      ...
      imu_msg.orientation.x = random(100)/100.0;
      imu_msg.orientation.y = random(100)/100.0;
      imu_msg.orientation.z = random(100)/100.0;
      imu_msg.orientation.w = random(100)/100.0;
      
      imu_msg.angular_velocity.x = random(100)/100.0;
      imu_msg.angular_velocity.y = random(100)/100.0;
      imu_msg.angular_velocity.z = random(100)/100.0;
  
      imu_msg.linear_acceleration.x = random(100)/100.0;
      imu_msg.linear_acceleration.y = random(100)/100.0;
      imu_msg.linear_acceleration.z = random(100)/100.0;
...
void update_odom(void){
  ...
  /* Fusing Odometry */
  deltaYaw = random(100)/100.0 - thetaYaw;
  thetaYaw = random(100)/100.0;
