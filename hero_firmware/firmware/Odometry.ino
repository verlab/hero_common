...
void Odometry::init(ros::NodeHandle &nh, String heroName, WheelEncoder &wheelEncoder, IMUSensor &imuSensor) {
  ...
  this->odomTopic = this->heroName + String("/odom");                       /* Update topic name */
  ...
  this->setOdomTopic = this->heroName + String("/set_odom");                                                                                                    /* Update service name */
...
void Odometry::compute() {
  ...
  this->odomMessage.pose.pose.position.x += deltaSteps * cos(this->poseMessage.theta + this->deltaTheta / 2.0);
  this->poseMessage.x = this->odomMessage.pose.pose.position.x;
  this->odomMessage.pose.pose.position.y += deltaSteps * sin(this->poseMessage.theta + this->deltaTheta / 2.0);
  this->poseMessage.y = this->odomMessage.pose.pose.position.y;
  this->odomMessage.pose.pose.position.z = 0.0;

  this->poseMessage.theta += deltaTheta; /* Expressed in radiant. */
  ...
  this->odomMessage.twist.twist.linear.x = (this->wheelEncoder->getMessage().right_speed_filtered + this->wheelEncoder->getMessage().left_speed_filtered) / 2.0;
  /* Wheel speed in m/s */
