...
bool PID_ATune::Runtime()
{
    ...
    if (stepCount > 4)
    {
      ...
      if ((avgStep1 > 1e-10) && (avgStep2 > 1e-10))
      {
        ...
        if (asymmetry > AUTOTUNE_STEP_ASYMMETRY_TOLERANCE)
        {
          ...
          if (fabs(deltaRelayBias) > workingOstep * AUTOTUNE_STEP_ASYMMETRY_TOLERANCE)
          {
  ...
  Serial.print(F("output start"));
  Serial.println(outputStart);
  Serial.print(F("state "));
  Serial.println(state);
#endif
  ...
  { 
    ...
    for (byte i = 2; i <= 4; i++)
    {
      ...
      double diff_val = val - lastPeaks[i - 1];
      inducedAmplitude += fabs( diff_val); 
      if (absMax < val)
      {
         absMax = val;
      }
    ...
    if (controlType == AMIGOF_PI)
    {
      ...
      double diff_phaseLag = phaseLag - CONST_PI * 130.0 / 180.0;
      if (fabs(diff_phaseLag) > (CONST_PI * 15.0 / 180.0))
      {
